...
class Evaluator:
    ...
    def GetPascalVOCMetrics(self, boundingboxes, IOUThreshold=0.5):
        ...
        for c in classes:
            ...
            r = {
                ...
                'total FP': np.sum(FP),
                'TP_array': TP,
                'probabilityOfDetection':[detet[2] for detet in dects],
                }
            ret.append(r)
        return ret
    ...
    def PlotPrecisionRecallCurve(self, classId, boundingBoxes, IOUThreshold=0.5, showAP=False, showInterpolatedPrecision=False, savePath=None, showGraphic=True):
        ...
        TP = result['TP_array']
        probabilityOfDetection = result['probabilityOfDetection']

        self.plotROCCurve(TP, probabilityOfDetection, classId)
    ...
    def plotROCCurve(self, hitOrMiss, probability, classId):
        """

        :param hitOrMiss: if detection is hit when iou > threshold - true positive array
        :param probability: softmax function value output
        :return: void
        """

        fpr, tpr, thresholds = metrics.roc_curve(hitOrMiss, probability, pos_label=1)
        auc = np.trapz(tpr, fpr)
        print('[ROC] - AUC:', auc, "({})".format(str(classId)))
        # Print ROC curve
        plt.plot(fpr, tpr, label='ROC')
        plt.plot(np.arange(0, 1.01, 0.01), np.arange(0, 1.01, 0.01), '--r', label='Random')
        plt.title('TPR x FPR curve \nClass: %s AUC: %f' % (str(classId), auc))
        plt.xlabel('FPR')
        plt.ylabel('TPR')
        plt.legend(shadow=True)
        plt.axis('square')
